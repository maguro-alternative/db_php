<html>
    ...
    <body>
        ...
        <?php
            if(isset($_POST['send'])) 
            {
                $escaped=htmlspecialchars($_POST['classid'] );
            ...
            $query=pg_query($dbconn,"SELECT * FROM students WHERE class={$escaped}");
        ?>
        <table border="1">
            <?php
                $idArray = array(101,102,103);
                if (in_array($escaped,$idArray) == false){
                    print "not rows";
                }
